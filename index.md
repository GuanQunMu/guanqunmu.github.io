<table border="0">
  <tr>
    ...
    <td width="65%">
      ...
      <p>I am keenly interested to peruse my Masters and Doctorate in scalable quantum computing systems from 2021. My interest currently meets in <b>quantum information processing in ion traps</b> and <b>simulation of physical processes in quantum computing models</b>.</p>
      <p> A short detail is as given below. </p>
    </td>
  </tr>
</table>
...
### **1. Designing the Control System for Zeeman Scan, Rabi Scan and Spin Echo Operations of Ca+ Trapped Ion Qubits**
...
<small>(a) Interacting with the 729 nm wavelength laser, the Ca + ion oscillate between the ground state and the D5/2 state. (b) The pulse sequence of spin-echo operation (UDD mode) generated by the Artiq (c) A preview of the dashboard and customized GUI shown on the PC. </small>  

**The Project in progress:** A new idea of the modified quantum lock-in amplifier is under discussion. This idea is proposed by me and has the advantage of preventing the unwanted resonance of the quantum state and high-frequency magnetic field signal by changing the laser pulse sequence driven on the ion. Please follow our update in the future.  

If you want to know about the details or to use this project in your experiment, please check its GitHub page: [https://github.com/GuanQunMu/IonTrap-WIPM](https://github.com/GuanQunMu/IonTrap-WIPM)
...
<small>(a)  Time evolution of the probability distribution with the quantum phase of a coherent state with α=3 (the picture is from Wikipedia). (b) The FPGA program: The binary bit string transmitted by the JTAG protocol can determine when to end the experiment; the signal from the PLL passes through the counter to determine the current time, waiting for the signal in the PMT to trigger.</small>  

**The Project in progress:** Simulate the ultra-fast qubit transition for 137Ba+ ions, involving the dipole transitions of S1/2 ↔ P1/2 and S1/2 ↔ P3/2. Calculated the Stark shift effect and spontaneous emission probability during a π pulse, which will guide our future experiments. Please follow our update.  
...
<small>(a) A two-dimensional array implementation of the surface code (form Prof. John Martinis's journal paper). (b) The fidelity of one logical qubit array. The x-axis represents the error rate of physical qubits gates, and the y-axis represents the number of errors of encoded samples in 100,000 Monte Carlo samples. Different colors represent different array sizes. It shows that the threshold is 0.105.</small>

If you want to know about the details or to run the surface code with it, please check its GitHub page: [https://github.com/GuanQunMu/Surface-Code](https://github.com/GuanQunMu/Surface-Code)
...
1 year FPGA Programming Experience (Verilog)  
